...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType outputSettings:(NSMutableDictionary *)outputSettings;
{
    ...
    previousFrameTime = kCMTimeNegativeInfinity; // 负无穷大
...
- (void)enableSynchronizationCallbacks;
{
    if (videoInputReadyCallback != NULL)
    {
        ...
        [assetWriterVideoInput requestMediaDataWhenReadyOnQueue:videoQueue usingBlock:^{
            ...
            {
                if( videoInputReadyCallback && ! videoInputReadyCallback() && ! videoEncodingIsFinished ) // 读取失败 却没有读取完毕
                {
...
- (void)createDataFBO;
{
    // framebuffer object 通常也被称之为 FBO，它相当于 renderbuffer(color, depth, stencil)的管理者，三大renderbuffer 可以附加到一个 FBO 上
    ...
    else
    {
        ...
        //将 movieRenderbuffer 装配到 GL_COLOR_ATTACHMENT0 这个装配点上
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, movieRenderbuffer);	
    }
    /*
     帧缓冲区对象提供了高效的切换机制：
     从帧缓冲区对象中断开先前的帧缓冲区关联图像，连接一个新的帧缓冲区关联图像到帧缓冲区对象上。
     切换帧缓冲区关联图像比切换帧缓冲区对象快多了。
     帧缓冲区对象提供了glFramebufferTexture2D()来切换2D纹理对象,glFramebufferRenderbuffer()来切换渲染缓冲区对象。
     */
...
//    NSLog(@"Movie writer framebuffer: %@", inputFramebufferToUse);
    ...
    // glFinish()作用也是将缓冲区中的指令(无论是否为满)立刻送给图形硬件执行，但是要等待图形硬件执行完这些指令才返回。
    glFinish();
}
    ...
    runAsynchronouslyOnContextQueue(_movieWriterContext, ^{
        ...
        else
        {
            ...
            else
            {
                ...
                /*
                 glReadPixels函数的参数说明，该函数总共有七个参数。
                 ...
                 第七个参数表示一个指针(读取到的数据保存到的地址)，像素数据被读取后，将被保存到这个指针所表示的地址。注意，需要保证该地址有足够的可以使用的空间，以容纳读取的像素数据。例如一幅大小为256*256的图象，如果读取其RGB数据，且每一数据被保存为GLubyte，总大小就是：256*256*3 = 196608字节，即192千字节。如果是读取RGBA数据，则总大小就是256*256*4 = 262144字节，即256千字节。
