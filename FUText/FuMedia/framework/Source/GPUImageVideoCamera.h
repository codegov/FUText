...
@interface GPUImageVideoCamera : GPUImageOutput <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
{
    ...
    AVCaptureDevice *_inputCamera;     // 摄像头
    AVCaptureDevice *_microphone;      // 麦克风
    AVCaptureDeviceInput *videoInput;  // 输入设备，如：前置或后置摄像头、麦克风等
	AVCaptureVideoDataOutput *videoOutput; // 视频数据输出

    BOOL capturePaused; // 是否暂停捕获
    GPUImageRotationMode outputRotation, internalRotation;
    dispatch_semaphore_t frameRenderingSemaphore; // 信号量
        
    BOOL captureAsYUV; //YUV是指亮度参量和色度参量分开表示的像素格式
    GLuint luminanceTexture, chrominanceTexture; // 亮度，色度
...
@property (readwrite, nonatomic, copy) NSString *captureSessionPreset; // 预设大小
...
@property (readonly, getter = isFrontFacingCameraPresent) BOOL frontFacingCameraPresent; // 是否是前置摄像头
@property (readonly, getter = isBackFacingCameraPresent) BOOL backFacingCameraPresent;   // 是否是后置摄像头

/// This enables the benchmarking mode, which logs out instantaneous and average frame times to the console
@property(readwrite, nonatomic) BOOL runBenchmark; // 开启log
...
@property(readwrite, nonatomic) BOOL horizontallyMirrorFrontFacingCamera, horizontallyMirrorRearFacingCamera; // 摄像头是否镜像
