...
- (BOOL)renderFrames:(UInt32)numFrames ioData:(AudioBufferList *)ioData
{
    ...
    if (_playing && _outputBlock )
    {
        ...
        else if (_numBytesPerSample == 2) // then we need to convert SInt16 -> Float (and also scale)
        {
            ...
            LoggerAudio(2, @"Buffer %u - Output Channels %u - Samples %u", (uint)ioData->mNumberBuffers, (uint)ioData->mBuffers[0].mNumberChannels, (uint)numFrames);
#endif
...
#ifdef DUMP_AUDIO_DATA
                dumpAudioSamples(@"Audio frames decoded by FFmpeg and reformatted:\n", ((SInt16 *)ioData->mBuffers[iBuffer].mData), @"% 8d ", numFrames, thisNumChannels);
#endif
            }
...
- (BOOL)activateAudioSession
{
    if (!_activated)
    {
        if (!_initialized)
        {
            OSStatus s = AudioSessionInitialize(NULL, kCFRunLoopDefaultMode, sessionInterruptionListener, (__bridge void *)(self));
            if (checkError(s, "Couldn't initialize audio session")) return NO;
            _initialized = YES;
        }
