...
@interface GPUImageMovieEx () <AVPlayerItemOutputPullDelegate>
{
    ...
    GLuint luminanceTexture, chrominanceTexture; // 亮度 和 色度
...
@implementation GPUImageMovieEx
...
- (void)startProcessing
{
    ...
    GPUImageMovieEx __block *blockSelf = self;
...
- (void)processAsset
{
    ...
    __unsafe_unretained GPUImageMovieEx *weakSelf = self;
...
- (void)processPlayerItem
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if (!CGSizeEqualToSize(_presetSize, CGSizeZero))
        {
            [pixBuffAttributes setObject:@(_presetSize.width) forKey:(id)kCVPixelBufferWidthKey];
            [pixBuffAttributes setObject:@(_presetSize.height) forKey:(id)kCVPixelBufferHeightKey];
        }
...
- (void)processMovieFrame:(CVPixelBufferRef)movieFrame withSampleTime:(CMTime)currentSampleTime
{
    ...
    if ([GPUImageContext supportsFastTextureUpload])
    {
        ...
        //Plane 使用三个数组分开存放YUV三个分量，就像是一个三维平面一样。
        if (CVPixelBufferGetPlaneCount(movieFrame) > 0) // Check for YUV planar inputs to do RGB conversion
        {
            ...
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // 水平 靠边线绘制一次
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE); // 垂直 靠边线绘制一次
